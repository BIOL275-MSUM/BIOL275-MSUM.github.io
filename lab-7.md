...
5.  Create a single graph that shows the distribution of penguin body masses using three histograms, one for each species; include vertical lines showing the median and mean body masses for each species
    ...
    -   Adding vertical lines for both the mean and median requires reshaping the summary data into long format using `pivot_longer()`
        -   You can put the name of the statistic, "Mean" or "Median", in a new column named **statistic**, and the values in a new column named **value**.
        -   When you add the `geom_vline()` you need to set the data argument to the vertical line dataset you just created
